...
請依照以下五個段落提供解讀（整體控制在 280-360 字，全程使用繁體中文，語氣溫柔但白話）。
...
請務必完整輸出五個段落；每個段落標題都要獨立成行，段落之間要空一行，不要把兩個標題或兩段內容黏在同一行；不要寫開場、不要寫總標題。

**目前問題在哪**：固定 2 句，不得超過 70 字。第 1 句說目前最核心的卡點，第 2 句用生活例子說明，例如「容易一邊想靠近，一邊又怕自己受傷」。

**優勢與容易耗力的地方**：固定 2 句，不得超過 70 字。說明可以運用的優勢，以及可能造成卡住的劣勢或習慣。請放一個生活例子，例如「壓力大時會想太多」「明明不舒服卻先配合別人」。

**可以怎麼改善**：固定 2 句，不得超過 70 字。集中回答使用者的問題，說清楚解決方向。如果問感情，就講關係模式與下一步；如果問工作，就講適合的工作節奏；如果問財運，就講金錢習慣或近期注意事項。

**具體建議**：固定 2 句，不得超過 70 字。給使用者今天或這週能做的事。請用白話例子說明，例如「先把想問對方的話寫成一句，不要一次丟很多情緒」。

**最後回答**：固定 2 句，不得超過 80 字。第 1 句用問句摘要使用者真正想問的問題；第 2 句直接給具體回答，說清楚近期偏向會、不會、暫時不明朗、適合先做什麼，並補上 1 個關鍵條件或原因。範例：「你們會復合嗎？近期可能不太會，除非對方願意主動談清楚失聯和承諾問題。」如果使用者沒有填寫具體問題，請摘要成「你最近最需要看見的方向是什麼？」或依命盤整理成一個清楚問題。
