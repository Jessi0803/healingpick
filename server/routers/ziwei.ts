...
const ZIWEI_PRIVATE_MESSAGE_STYLE = `共同語氣：
- 用繁體中文，像在 LINE 裡把命盤翻成白話給朋友聽：溫柔、具體、有判斷，但不下絕對斷言。
- 先回答使用者真正想問的方向，再補命盤原因和現實例子；不要寫成命理文章或漂亮但聽不懂的療癒文。
- 符號頻率接近「範例2」：emoji、～、！可以自然出現，約每 1-2 行 1 個；情緒重點句可以更活潑一點，但只放在句尾或轉折處，不放標題，不連續堆疊。
- 段落像自然私訊：短句分行，每行一個完整想法；主題轉換空一行，不要把 3-5 句擠成一個大段落。
- 避免空泛鼓勵；所有提醒都要落到現實行為，例如今天先問清楚、這週先觀察、不要一次丟太多情緒。`;
...
請依照以下四個段落提供解讀，整體控制在 400-500 字。
${ZIWEI_PRIVATE_MESSAGE_STYLE}
每個段落都要用 Markdown 粗體標出 1 個真正重要的判斷或行動重點，而且每段剛好粗體 1 個短句。
粗體要具體，例如「**先觀察對方是否願意主動約時間**」，不要寫「**相信自己**」「**慢慢來**」這類空話。
...
${ZIWEI_PRIVATE_MESSAGE_STYLE}
正文每段剛好 1 個具體粗體短句。禁止寒暄開場，第一行必須直接是「${ZIWEI_SECTION_START}」。`,
          },
...
追問規則：
- 控制在 260-420 字，不重新排命盤，也不要假裝有新的出生資料。
- 第一段直接回答追問，不要籠統安慰。
- 引用命盤中的 2-3 個具體重點，例如命宮、身宮、主要宮位、主星或原解讀判斷。
- 說明為什麼這樣判斷，並把命盤訊息和使用者問題連在一起。
- 最後給 2 個今天或這週可執行的建議。
- 不要使用 Markdown 標題、編號或項目符號；不要出現「命中注定」「一定會」「絕對不會」。`;
