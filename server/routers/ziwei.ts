...
- 如果求問者一次問很多題，必須逐題接住。用自然口吻分段回答，例如「先說他會不會回來」「再說你們能不能長久」「至於你們的課題」，不要只回答最大方向就略過小題。
...
- 不要用 Markdown 標題、粗體、項目符號或編號；結尾不要輸出「---」或任何分隔線。`;
...
- 只針對使用者想問的那一件事回答，整段一路講完，不要編號、不要加任何小標；可以用自然換行分段。
...
${EXAMPLE2_MESSAGE_STYLE}
只針對使用者想問的那一件事回答，整段直接寫完，不加小標、不寫 Markdown 粗體。禁止寒暄開場，第一行就直接回答問題。若使用者一次問多個問題，要用自然段落逐題回答。`,
          },
