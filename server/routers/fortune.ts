...
export function buildFallbackFortune({
  ...
  return {
    overall: `${signLabel}今天比較適合先把節奏放穩。你可能會想等一切更確定再開始，但真正卡住你的不是能力，而是把門檻設得太精準；**先挑一件最想推進的事做十分鐘**，比在腦中反覆排演更有用。`,
    overallScore: scoreFromSeed(seed, 11),
    love: `感情上容易因為小細節想太多，像對方晚回、語氣變淡，就開始自己補完劇情。今天不用一次把心裡話攤開，**先丟出一個小邀請或一句明確關心**，比悶著等對方猜更有推進力。`,
    ...
    health: `身體狀態需要少一點硬撐，尤其是肩頸、腸胃或睡眠容易被焦慮牽動。今天如果覺得累，**先補水並走路十分鐘**，讓緊繃的注意力慢慢鬆開。`,
    ...
    crystalReason: `${guide.crystal}適合搭配${moonPhase.name}，陪你**把情緒和下一步行動分開**，不用一次承擔全部。`,
    advice: `今天先做一個很小但能落地的整理：**${guide.action}**。這會讓你比較知道該往哪裡推進、又該在哪裡先穩住。`,
...
- 寫法像「今天的提醒」，但不要每段都用同一種模板；可以有的段落先給畫面、有的先點出心理聲音、有的先給小提醒
- 避免每段都寫成「你可能...但...建議...例如...」；句型要自然，有一點像真的在替朋友看今天的狀態
- 今天的月相名稱最多在 overall、crystalReason、advice 其中 2 個欄位明確出現；其他欄位可改用「起點感」「整理的氣氛」「適合收尾的節奏」等語感，不要反覆堆同一個月相詞
- 星座特性要寫得像使用者自己的內在聲音或日常慣性，例如「等我整理好再開始」「我是不是還不夠完整」「先把小地方修好」；不要只列特質名詞
...
{
  "overall": "整體運勢描述（2-3句；結合月相能量與星座內在慣性；說出今天容易卡住的心理聲音，以及可以怎麼調整，例如先把一件事拆成小步驟）",
  "overallScore": 整體分數（1-10的整數）,
  "love": "愛情運勢描述（2-3句；用日常互動語氣說出容易腦補或卡住的地方，以及可以怎麼做，例如先丟出一個小邀請，不要只用回訊息速度判斷）",
  "loveScore": 愛情分數（1-10的整數）,
  "career": "事業財運描述（2-3句；同時包含工作與財務，說出容易卡住的原因，以及可以怎麼做，例如收尾任務、整理報價、確認付款或避開衝動消費）",
  "careerScore": 事業分數（1-10的整數）,
  "health": "健康運勢描述（2-3句；說出今天身體可能反映在哪裡，以及可以怎麼照顧，例如補水、伸展、早點休息或散步十分鐘）",
