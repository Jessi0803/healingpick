...
- 偶爾可以用 1 個可愛符號（例如 ✦、☽、🐾、♡）或「～」點綴，但整份最多 1-2 次，不要每段都有；不要出現「喵」這個字
- 使用繁體中文，輕盈有溫度但不文謅謅
- 運勢要實際、給得出具體感受，不空泛
- 整體所有文字控制在 180-240 字，不要超過 260 字
...
- overall、love、career、health、advice 都要各自放一個短例子，但每欄盡量 1-2 句，不要讓整體超過字數限制
...
{
  "overall": "整體運勢描述（1-2句；結合月相能量；說出今天容易卡住的原因，以及可以怎麼調整，例如先整理待辦）",
  "overallScore": 整體分數（1-10的整數）,
  "love": "愛情運勢描述（1-2句；說出容易卡住的原因，以及可以怎麼做，例如不要只用回訊息速度判斷）",
  "loveScore": 愛情分數（1-10的整數）,
  "career": "事業財運描述（1-2句；說出容易卡住的原因，以及可以怎麼做，例如適合收尾或整理資料）",
  "careerScore": 事業分數（1-10的整數）,
  "health": "健康運勢描述（1-2句；說出健康狀態容易卡住的原因，以及可以怎麼做，例如早點休息或散步十分鐘）",
  ...
  "advice": "今日行動建議（1句；結合月相指引；給可以照做的小行動，說明能改善什麼）",
  "moonPhase": "${moonPhase.name}",
  "moonSymbol": "${moonPhase.symbol}"
}`;
