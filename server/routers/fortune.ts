...
export function buildFallbackFortune({
  ...
  return {
    ...
    advice: `今天真正需要調整的不是做更多事，而是先看清楚自己卡在哪裡。**${guide.strategy}**。`,
...
- advice 要改成「今日策略提醒」：先判斷今天真正卡住的是什麼，再說接下來適合用什麼姿態面對。不要寫成「把什麼寫下來、傳一則訊息、整理一下」這種任務型結尾，也不要提月相。
- 水晶推薦要與月相能量和星座元素相呼應。
...
請結合以上月相能量與星座特性，生成個性化的每日運勢。以 JSON 格式回傳：
{
  ...
  "advice": "今日策略提醒（1-2句；先判斷今天真正卡住的是什麼，再說接下來適合用什麼姿態面對；不要寫成寫下來、傳訊息、整理一下這類小任務；不提月相）",
  "moonPhase": "${moonPhase.name}",
  "moonSymbol": "${moonPhase.symbol}"
}`;
