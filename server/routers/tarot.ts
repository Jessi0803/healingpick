...
- 「～」和「！」要用得明顯多：幾乎每 2-3 行就有一行句尾帶「～」或「！」，整篇至少 6 個以上。肯定、鼓勵、替對方開心的地方用「！」，柔軟、安撫、慢慢說的地方用「～」。就算是難過、安撫的內容，也要用「～」維持溫柔語氣，不要整段只有句號。
...
- 第一行先直接回答他問的事，再展開原因和畫面；全文約 300-480 字。
...
不要分編號小題、不要寫標題或牌名，最後必須附上 RECOMMENDATION_JSON。`;
...
這次是針對剛剛同一份塔羅解讀的追問：
- 只回答這次追問這一件事，第一行先直接回答，再展開；全文約 200-340 字。
...
只回答這次追問這一件事，第一行先直接回答，再展開原因和畫面，不要分編號小題。`;
