...
- 開頭先用 1-2 句白話回應對方現在的狀態，像朋友幫他整理問題，不要只說抽象能量
- 接著「每一張牌」各自一個小區塊，格式固定為：
  ### {位置} · {牌名}（正位 或 逆位）
  下面用 1-2 句白話解讀，並盡量連回求問者的主題
- 五張牌都講完後，加一個「### ✦ 整體訊息」區塊，用 2-3 句把牌串起來，說清楚目前卡住的原因、忽略的盲點、接下來可以怎麼面對
- 最後加「### 🐾 給你的話」區塊，給 2-3 句具體建議，最好有生活例子，例如「不要只看他回訊息快不快，可以觀察他有沒有主動約時間、願不願意把話說清楚」
- 少用「能量流動」「宇宙安排」「內在課題」這類抽象詞；如果使用，後面一定要用白話解釋
- 每個區塊都要精簡，全部加起來大約 450 字左右`;
