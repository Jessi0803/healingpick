...
- emoji、驚嘆號和「～」請依照語氣自然判斷何時加入，不需要刻意限制數量；可以比一般解讀活潑一點，但不要為了可愛而連續堆符號
...
- 段落格式請參考「範例2」截圖的自然私訊節奏：短句一行一行往下走，主題轉換時空一行，不要把 5-6 句擠成同一個大段落
- 每一行只放一個完整想法；如果一句超過 30-35 個中文字，請拆成兩行，但不要拆到讀起來像清單
...
- 不要使用項目符號、編號、Markdown 標題或粗體；只用自然換行和空行做分段
- 多用短句，少用長逗號串接；一句話盡量不超過 35 個中文字
- emoji、驚嘆號和「～」請依照語氣自然判斷何時加入，不需要刻意限制數量；可以比一般回覆活潑一點，但不要為了可愛而連續堆符號
