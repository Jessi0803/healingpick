  ...
  interpret: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const cardsSummary = input.cards
        .map(
          (c, i) =>
            `${i + 1}. ${c.position}：${c.name}（${c.en}）${c.reversed ? "【逆位】" : "【正位】"} — ${c.reversed ? "逆位含義：" + c.meaning : "含義：" + c.meaning}`
        )
        .join("\n");
...
- 可以很偶爾用「～」或 1 個溫柔表情符號（例如 🐾、🌙、✨）讓語氣有溫度，但整篇最多 1-2 次，必須自然、克制，不要放在標題裡，也不要每段都用
...
- 不要寫任何開場白、寒暄或總起句，第一段必須直接從「# 第 1 張 · {牌名}（正位 或 逆位）」開始
- 每一張牌各自一個小區塊，格式固定為：
  # 第 {序號} 張 · {牌名}（正位 或 逆位）
  下面用白話解讀，必須連回求問者的主題。不要只翻譯牌義，也不要寫成玄學句子
- 輸出順序固定為：第 1 張、第 2 張、第 3 張、第 4 張、第 5 張
- 第 1 張到第 4 張每段固定 2 句：第 1 句說這張牌對問題的狀態與判斷，第 2 句用「例如：」給一個短例子。每段不得超過 70 字，不要補第三句，也不要在這四段展開行動做法
- 第 1 張到第 4 張都必須各有 1 個「例如：」生活例子；例子只放在第 2 句，要短，一句帶過，用來補足可觀察訊號，不要展開成長篇故事
...
- 不要加「# ✦ 整體訊息」區塊
- 不要加「# 🐾 給你的話」區塊；所有鼓勵、解決方向和具體行動都整合在第 5 張牌裡
- 只有第 5 張段落需要集中寫改善方向與具體做法，並且一定要包含 1 個「例如：」開頭、這週可以做的小行動
...
6. 不要寫開場白，第一行必須直接是「# 第 1 張 · {牌名}（正位 或 逆位）」
7. 不要把比喻當例子；「像貓咪」「像星星」這種只能當形容，不能算生活例子
8. 不要輸出「整體訊息」或「給你的話」，請把總結、鼓勵與具體行動放進第 5 張段落
9. 輸出順序固定為：第 1 張、第 2 張、第 3 張、第 4 張、第 5 張
10. 第 1 張到第 4 張每段固定 2 句：第 1 句說這張牌代表的狀態與判斷，第 2 句以「例如：」開頭給短例子；每段不得超過 70 字，不要補第三句，也不要每段都寫行動做法
11. 第 5 張段落一定要包含「直白結論」「解決方向」「這週能做的一個行動」三個重點，並用「例如：」給出具體做法`;
