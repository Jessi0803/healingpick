...
- 只針對求問者問的那一件事回答，整段一路講完；不要分成編號小題，也不要自己加小標。
...
- 如果求問者一次問很多題，必須逐題接住。用自然口吻分段回答，例如「先說他心裡有沒有別人」「再說他刪掉聯繫這件事」「至於現在斷聯是不是好的」，不要只回答最大方向就略過小題。
...
- 每行只講一個想法，而且要短，一行盡量不超過 20 個中文字；句子一長就拆成兩三行，不要用「，」把好幾個想法串成一長句。換到下一個意思時空一行。
  ...
  ✓ 你們有點像一個負責衝、一個負責穩～
  ✓ 互補很好 但也很容易卡
- 「～」和「！」像範例2那樣自然穿插就好：柔軟、慢慢說的句子句尾用「～」，肯定、鼓勵、替對方開心的地方用「！」。大約每 3-4 行才出現一個，不要每一行都用、也不要連續好幾段都沒有；難過或安撫的內容以「～」為主。
...
- 不要用「相信你的直覺、傾聽內在的聲音、對能量的敏感度」這種靈性話當結尾；結尾收一個具體的小動作。
...
這次是五牌陣的完整解讀：
- 只針對求問者問的那一件事回答，整段一路講完，不要分成編號小題或加小標。
- 第一行先直接回答他問的事，再展開原因和畫面；全文約 300-480 字。不要用「牌面沒有直接指向」「你現在最需要的不是」當第一句。
- 如果求問者一次問多個問題，每個問題都要在自然段落裡回答到，不要只抓主軸。
...
如果問題裡有多個小題，請用自然口吻逐題接住，但不要編號或加標題。
不要分編號小題、不要寫標題或牌名，不要輸出分隔線，最後必須附上 RECOMMENDATION_JSON。`;
...
- 如果追問裡有多個小題，每個問題都要在自然段落裡回答到，不要只抓主軸。
- 牌只放在背後判斷，不要逐張解牌，正文不要提牌名。`;
...
請根據以上五張牌，用上面範例那種 LINE 私訊語感寫完整解讀。
只針對「求問者的問題」這一件事回答，第一行先直接回答，再展開原因和畫面。
如果追問裡有多個小題，請用自然口吻逐題接住，但不要編號或加標題。
不要分編號小題、不要寫標題或牌名，不要輸出分隔線。`;
