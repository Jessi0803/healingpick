...
import { InsertReading, InsertTreeholeSession, readings, treeholeSessions, InsertUser, users } from "../drizzle/schema";
...
export async function saveReading(data: InsertReading): Promise<void> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot save reading: database not available");
    return;
  }
...
export async function saveTreeholeSession(data: InsertTreeholeSession): Promise<void> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot save treehole session: database not available");
    return;
  }
