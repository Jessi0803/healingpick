...
function RitualReviewCompanion({ onOpen }: { onOpen: () => void }) {
  ...
  return createPortal(
    <div className="fixed bottom-5 right-4 z-40 flex max-w-[calc(100vw-1.5rem)] items-end gap-2 sm:bottom-7 sm:right-6">
      <button
        ...
        aria-controls="ritual-feedback-dialog"
  ...
  useEffect(() => {
    ...
    const onPointerDownCapture = (event: PointerEvent) => {
      ...
      if (
        ...
        target.closest('[data-ritual-lightbox-close="true"]')
      ) {
  ...
  return (
    <PageLayout>
      ...
      {!isReviewsOpen && !selectedReview && (
        <RitualReviewCompanion onOpen={() => setIsReviewsOpen(true)} />
      )}
        ...
        createPortal(
          ...
          >
            <div className="relative z-10 flex items-start justify-between gap-4 text-[#FAF7F4]">
              ...
              <button
                type="button"
                data-ritual-lightbox-close="true"
                onPointerDown={(event) => {
                  event.preventDefault();
                  event.stopPropagation();
                  closeLightbox();
