...
const noticeGroups = [
  ...
  },
  {
    ...
    items: [
      "客製商品製作約 7-14 個工作天。",
      "如遇特殊節日或訂單較多，出貨時間將依公告為準。",
    ],
  },
];

const standardCrystalNotice =
...
const designNotes = [
  ...
  standardCrystalNotice,
];

type ProductCareNoticeVariant = "standard" | "custom" | "crystal-only";
...
export default function ProductCareNotice({
  ...
  variant?: ProductCareNoticeVariant;
}) {
  const isCustom = variant === "custom";
  const isCrystalOnly = variant === "crystal-only";
  const visibleNoticeGroups = isCustom
    ? noticeGroups
    : noticeGroups.filter(group => group.eyebrow !== "Returns");
  ...
  if (isCrystalOnly) {
    return (
      <section className="mx-auto mb-10 mt-14 max-w-5xl animate-fade-in-up">
        <div className="rounded-3xl border border-[#D1BE9B]/24 bg-[#F2EDE8]/58 p-5 shadow-[0_14px_42px_rgba(209,190,155,0.10)] md:p-6">
          <p
    ...
    <section className="mx-auto mb-10 mt-14 max-w-5xl animate-fade-in-up">
      ...
      <div className="mb-4 rounded-3xl border border-[#D1BE9B]/24 bg-white/55 p-6 shadow-[0_14px_42px_rgba(209,190,155,0.10)] md:p-7">
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:gap-7">
          ...
          <div className="flex-1">
            <p
              className="mb-3 text-[12.5px] leading-[1.9] tracking-[0.04em] text-[#31353A]/70"
              style={bodyStyle}
            >
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {visibleNoticeGroups.map(({ title, eyebrow, icon: Icon, items }) => (
          <div
            key={title}
            className="rounded-2xl border border-[#D1BE9B]/20 bg-white/50 p-5"
          >
      ...
      <div className="mt-4 rounded-3xl border border-[#D1BE9B]/24 bg-[#F2EDE8]/58 p-6 shadow-[0_14px_42px_rgba(209,190,155,0.10)] md:p-7">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {visibleDesignNotes.map(note => (
            <div
              key={note}
              className="flex items-start gap-3 rounded-2xl bg-white/48 px-4 py-3"
            >
