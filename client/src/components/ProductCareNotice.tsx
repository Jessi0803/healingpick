...
const designNotes = [
  ...
  '✦ 每款手鍊皆選用天然水晶製作，因自然形成，每顆水晶的色澤、紋理、透度及內含物皆略有不同，因此收到的作品會與展示照片存在些微差異',
];
    ...
    <section className="mx-auto mb-10 mt-14 max-w-5xl animate-fade-in-up">
      ...
      <div className="mb-6 flex flex-col gap-3 border-t border-[#D1BE9B]/20 pt-9 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="text-xl tracking-[0.14em] text-[#31353A] md:text-2xl" style={{ fontFamily: 'Noto Serif TC, serif', fontWeight: 300 }}>
            購買與客製注意事項
          </h2>
        </div>
        <p className="max-w-md text-[11.5px] leading-[1.8] tracking-[0.06em] text-[#31353A]/58" style={bodyStyle}>
          下單前請先閱讀，讓尺寸、天然水晶特性與後續服務都能被清楚確認。
        </p>
      </div>

      {/* Natural crystal — full-width intro banner */}
      <div className="mb-4 rounded-3xl border border-[#D1BE9B]/24 bg-white/55 p-6 shadow-[0_14px_42px_rgba(209,190,155,0.10)] md:p-7">
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:gap-7">
          <div className="flex items-center gap-3 md:w-48 md:flex-shrink-0">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-[0.24em] text-[#A38D6B]" style={eyebrowStyle}>
                Natural Crystal
              </p>
              <h3 className="text-[15px] tracking-[0.12em] text-[#31353A]" style={eyebrowStyle}>
                天然水晶特性
              </h3>
            </div>
          </div>
          <div className="flex-1">
            <p className="mb-3 text-[12.5px] leading-[1.9] tracking-[0.04em] text-[#31353A]/70" style={bodyStyle}>
            ...
            <p
              className="inline-block rounded-full bg-[#D1BE9B]/16 px-4 py-1.5 text-[12px] tracking-[0.06em] text-[#7B674B]"
              style={eyebrowStyle}
            >
              以上皆屬天然現象，非瑕疵。
            </p>
        ...
        {visibleNoticeGroups.map(({ title, eyebrow, icon: Icon, items }) => (
          <div key={title} className="rounded-2xl border border-[#D1BE9B]/20 bg-white/50 p-5">
            <div className="mb-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.22em] text-[#A38D6B]" style={eyebrowStyle}>
                  {eyebrow}
                </p>
                <h3 className="text-[14px] tracking-[0.12em] text-[#31353A]" style={eyebrowStyle}>
                  {title}
                </h3>
              </div>
            </div>
            <ul className="space-y-2.5">
              {items.map((item) => (
                <li key={item} className="flex items-start gap-2.5">
                  ...
                  <span className="text-[12px] leading-[1.85] tracking-[0.04em] text-[#31353A]/68" style={bodyStyle}>
                    {item}
                  </span>
                </li>
              ))}
      ...
      {/* Design — full-width */}
      <div className="mt-4 rounded-3xl border border-[#D1BE9B]/24 bg-[#F2EDE8]/58 p-6 shadow-[0_14px_42px_rgba(209,190,155,0.10)] md:p-7">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {visibleDesignNotes.map((note) => (
            <div key={note} className="flex items-start gap-3 rounded-2xl bg-white/48 px-4 py-3">
              ...
              <span className="text-[12px] leading-[1.85] tracking-[0.04em] text-[#31353A]/70" style={bodyStyle}>
                {note}
              </span>
            </div>
          ))}
